...
void VulkanApplication::pickPhysicalDevice()
{
	...
	for (const auto& device : devices)
	{
		if (isDeviceSuitable(device))
		{
			...
			msaaSamples = vulkan::util::getMaxUsableSampleCount(physicalDevice);
			break;
		}
	}
...
{
	auto indices = vulkan::util::QueueFamilyIndices::fromDeviceAndSurface(device, surface);

	bool swapChainAdequate = false;
	bool extensionsSupported = vulkan::util::checkDeviceExtensionSupport(device, deviceExtensions.data(), deviceExtensions.size());
...
{
	auto indices = vulkan::util::QueueFamilyIndices::fromDeviceAndSurface(physicalDevice, surface);
...
void VulkanApplication::createSwapChain()
{
	...
	VkExtent2D extent = vulkan::util::chooseSwapExtent(swapChainSupport.capabilities, framebufferWidth, framebufferHeight);

	uint32_t requestImageCount = swapChainSupport.capabilities.minImageCount + 1;

	if (swapChainSupport.capabilities.maxImageCount > 0 && requestImageCount > swapChainSupport.capabilities.maxImageCount)
	{
...
{
	auto vertShaderCode = readFile("shaders/triangle.vert.spirv");
	auto fragShaderCode = readFile("shaders/triangle.frag.spirv");
...
void VulkanApplication::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
	...
	allocInfo.memoryTypeIndex = vulkan::util::findMemoryType(physicalDevice, memRequirements.memoryTypeBits, properties);
...
uint32_t VulkanApplication::loadModelFromFile(const char* modelPath)
{
	...
	createVertexBuffer(vertices.data(), vertices.size(), mesh);
	createIndexBuffer(indices.data(), indices.size(), mesh);

	return meshIndex;
}

void VulkanApplication::createVertexBuffer(const Vertex* vertices, size_t count, Mesh& meshOut)
{
	...
	createBuffer(bufferSize, stagingUsage, stagingFlags, stagingBuffer, stagingBufferMemory);
	...
	createBuffer(bufferSize, usage, flags, meshOut.vertexBuffer, meshOut.vertexBufferMemory);

	copyBuffer(stagingBuffer, meshOut.vertexBuffer, bufferSize);

	vkDestroyBuffer(device, stagingBuffer, nullptr);
	vkFreeMemory(device, stagingBufferMemory, nullptr);
}

void VulkanApplication::createIndexBuffer(const uint16_t* indices, size_t count, Mesh& meshOut)
{
	...
	VkMemoryPropertyFlags flags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
	createBuffer(bufferSize, usage, flags, meshOut.indexBuffer, meshOut.indexBufferMemory);

	copyBuffer(stagingBuffer, meshOut.indexBuffer, bufferSize);
...
void VulkanApplication::createUniformBuffers()
{
	...
	for (size_t i = 0; i < swapchainImageCount; i++)
	{
		...
		createBuffer(bufferSize, usage, flags, uniformBuffers[i], uniformBuffersMemory[i]);
	}
}

uint32_t VulkanApplication::createRenderObject(const glm::mat4& transform, uint32_t modelId)
{
	uint32_t index = objects.size();
	objects.push_back(RenderObject{ transform, modelId });
	return index;
}
...
void VulkanApplication::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples,
...
void VulkanApplication::createTextureImage()
{
	...
	VkMemoryPropertyFlags flags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
	createBuffer(imageSize, usage, flags, stagingBuffer, stagingBufferMemory);
...
{
	auto queueFamilyIndices = vulkan::util::QueueFamilyIndices::fromDeviceAndSurface(physicalDevice, surface);
...
void VulkanApplication::recordCommandBuffer()
{
	...
	renderPassInfo.framebuffer = swapChainFramebuffers[currentSwapchainImage];
...
{
	presentFinishedSemaphores.resize(swapchainImageCount, VK_NULL_HANDLE);
...
{
	// Wait for the previous instance of currentFrame to finish
	vkWaitForFences(device, 1, &frameFences[currentFrame], VK_TRUE, UINT64_MAX);
	vkResetFences(device, 1, &frameFences[currentFrame]);

	//vkAcquireNextImageKHR will signal this semaphore when it is ready for use on the GPU
	VkResult result = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, presentFinishedSemaphores[currentSwapchainSemaphore], VK_NULL_HANDLE, &currentSwapchainImage);
	...
	VkSemaphore* imageAvailableSemaphore = &presentFinishedSemaphores[currentSwapchainSemaphore];
	VkSemaphore* renderFinishedSemaphore = &renderFinishedSemaphores[currentSwapchainSemaphore];
	...
	currentSwapchainSemaphore = (currentSwapchainSemaphore + 1) % swapchainImageCount;
}
